...
tomando GLOW como template · cambia categoría-específicos · mantiene pricing B2B estándar.
"""
import os, re, subprocess, sys
from pathlib import Path

ROOT = Path('/Users/end/Desktop/tdi-peptidos/peptiqmx/catalogos')
TEMPLATE = ROOT / 'PEPTIQ-GLOW-Catalogo-2026.html'
...
CATEGORIES = {
    'WOLVERINE': {
        ...
        'cover_lede': 'Péptidos research-grade para medicina deportiva, fisioterapia, traumatología y protocolos de recuperación post-cirugía. Con COA Janoshik verificado por lote — único en México.',
        'line_intro_title': 'Recuperación tisular',
        'line_intro_title_em': 'avanzada.',
        'line_intro_lede': 'Línea de péptidos BPC-157, TB-500 y moduladores inflamatorios. Formulados para protocolos de lesión de tendón, ligamento, intestino y recuperación post-quirúrgica.',
        'line_eyebrow': 'Línea WOLVERINE · Tissue Repair',
        'category_name': 'Wolverine',
        'target': 'medicina deportiva, traumatología y fisioterapia',
        'euromonitor_lever_1': ('Lever #1 · Medicina deportiva en expansión', 'El mercado global de recuperación tisular crece +7% CAGR. Post-COVID, clínicas de fisioterapia y deportiva han triplicado demanda de protocolos científicamente validados. Los que integran péptidos research-grade capturan 40% más ticket promedio.'),
        'euromonitor_lever_2': ('Lever #2 · BPC-157 · el péptido más estudiado', 'Más de 1,200 estudios preclínicos desde 1991. Acelera angiogénesis, reduce tiempo de recuperación tendón 40-60%, modula inflamación sistémica sin efecto inmunosupresor. Considerado el gold standard en regeneración peptídica.'),
        'euromonitor_lever_3': ('Lever #3 · TB-500 y sinergias', 'Thymosin Beta-4 fragment complementa BPC-157 en cascada regenerativa. Juntos cubren las dos vías principales: migración celular (TB-500) + angiogénesis (BPC-157). Stack obligatorio en protocolos post-quirúrgicos premium.'),
        'products': [
    ...
    'HIGHLANDER': {
        ...
        'cover_lede': 'Péptidos research-grade para medicina regenerativa, longevity clinics y protocolos biological age reversal. Con COA Janoshik verificado por lote — único en México.',
        ...
        'target': 'medicina regenerativa, anti-aging y longevity clinics',
    ...
    'PRIME': {
        ...
        'cover_lede': 'Péptidos research-grade para endocrinología, performance, GH optimization y composición corporal en hombres y mujeres 35+. Con COA Janoshik verificado por lote.',
        ...
        'category_name': 'Prime',
        'target': 'endocrinología, medicina deportiva y wellness performance',
    ...
    'SHRED': {
        ...
        'cover_lede': 'Péptidos research-grade para clínicas de obesidad, nutrición funcional y protocolos GLP-1. Alternativa research-grade a Ozempic con preservación muscular. Con COA Janoshik verificado por lote.',
        ...
        'category_name': 'Shred',
        'target': 'clínicas de obesidad, nutrición funcional y medicina metabólica',
        ...
        'products': [
            ...
            {'name':'Tirzepatide', 'dose':'30mg', 'price':8999, 'subtitle':'Dual GLP-1/GIP · Mounjaro alternative', 'image':'products-v2/tirzepatide-studio.png', 'category':'Dual Agonist · 30mg',
    ...
    'SPA': {
        ...
        'cover_lede': 'Péptidos research-grade para spa médico, clínicas wellness premium y protocolos 360° multi-servicio. Con COA Janoshik verificado por lote — único en México.',
        ...
        'target': 'spa médico, wellness clinics y medspa premium',
  ...
  <div class="product-hero">
    ...
    <div class="product-header">
      ...
      <div class="product-price-box">
        <div class="pp-price" style="text-decoration:line-through;color:#888;font-size:18px;font-weight:400">${p['price']:,} MXN <span style="font-size:11px;letter-spacing:1.2px;color:#888;text-decoration:none">RETAIL</span></div>
        <div class="pp-price" style="font-size:32px;color:#111;font-weight:600;margin-top:6px">${int(p['price']*0.75):,} MXN <span style="font-size:11px;letter-spacing:1.2px;color:#111;font-weight:500">CLÍNICA · 25% OFF</span></div>
        <div class="pp-unit">Por vial · {p['dose']} liofilizado</div>
        <div class="pp-b2b">Tier 1 (entrada): ${int(p['price']*0.9):,} · Tier 2 (estándar): ${int(p['price']*0.8):,} · Tier 3 (volumen): ${int(p['price']*0.75):,} · Tier ELITE: ${int(p['price']*0.65):,}</div>
      </div>
    </div>
  </div>
...
for cat_key, cfg in CATEGORIES.items():
    ...
    html = html.replace('Línea GLOW · Medicina estética · Dermatología', cfg['line_eyebrow'])
    html = html.replace('La ciencia de la<br><em>piel radiante.</em>', f'{cfg["title"]}<br><em>{cfg["title_em"]}</em>')
    html = html.replace('Péptidos research-grade para medicina estética, dermatología y protocolos de longevidad dérmica. Con COA Janoshik verificado por lote — único en México.', cfg['cover_lede'])
    ...
    if deep:
        ...
        # Replace 3 levers (callouts page 3) — FIX: use .*? instead of [^<]+ to match inline tags
        ...
        html = re.sub(
            r'(<div style="display:grid;grid-template-columns:repeat\(2,1fr\);gap:14px;margin-top:18px">\s*)(?:<div class="callout" style="margin:0">.*?</div>\s*){6}(\s*</div>\s*</div>\s*<div class="page-footer">\s*<span class="pf-left">PEPTIQ MX · Línea [A-Z]+ · Casos de uso</span>)',
...
'''
        ...
        # === NEW: Replace category-intro lede paragraph (page 5) — was hardcoded GLOW peptides ===
        html = re.sub(
            r'<p style="font-size:14\.5px;color:rgba\(255,255,255,0\.78\);max-width:600px;line-height:1\.6;font-weight:300">.*?</p>',
            f'<p style="font-size:14.5px;color:rgba(255,255,255,0.78);max-width:600px;line-height:1.6;font-weight:300">{deep["intro_lede"]}</p>',
            html, count=1, flags=re.DOTALL
        )

        # === NEW: Replace tier pricing table (page 13) per-line products ===
        try:
            from lines_extra import EXTRA
            extra = EXTRA.get(cat_key, {})
        except ImportError:
            extra = {}

        if extra.get('tier_products'):
            new_rows = ''
            for i, (name, sub, retail) in enumerate(extra['tier_products']):
                t1 = int(retail * 0.9)
                t3 = int(retail * 0.75)
                cls = 'pt-pricing-row featured' if i % 2 == 0 else 'pt-pricing-row'
                new_rows += f'''      <div class="{cls}">
        <div class="p-name">{name}<small>{sub}</small></div>
        <div class="p-price">${retail:,}</div>
        <div class="p-b2b">${t1:,}</div>
        <div class="p-vip">${t3:,}</div>
      </div>
'''
            html = re.sub(
                r'(<div class="pt-body">\s*)(?:<div class="pt-pricing-row[^"]*">.*?</div>\s*</div>\s*){5,}',
                lambda m: m.group(1) + new_rows,
                html, count=1, flags=re.DOTALL
            )

        # === NEW: Replace COA samples (page 15) per-line ===
        if extra.get('coa_samples'):
            new_coas = ''
            for s in extra['coa_samples']:
                new_coas += f'''    <div class="coa-card">
      <div class="coa-seal">Muestra · {s['name'].split()[0]}</div>
      <h4>{s['name']} · Lote {s['lot']}</h4>
      <div class="coa-specs">
        <div><b>HPLC Pureza</b><span>{s['purity']}</span></div>
        <div><b>Masa molecular</b><span>{s['mass']}</span></div>
        <div><b>Endotoxinas</b><span>{s['endo']}</span></div>
        <div><b>Aspecto</b><span>{s['color']}</span></div>
      </div>
      <p style="font-size:10px;color:var(--mute);margin-top:6px">Analizado {s['date']} · Janoshik Laboratory · ISO-17025</p>
    </div>
'''
            html = re.sub(
                r'(<div class="coa-grid">\s*)(?:<div class="coa-card">.*?</div>\s*){3,4}\s*(</div>)',
                lambda m: m.group(1) + new_coas + '  ' + m.group(2),
                html, count=1, flags=re.DOTALL
            )

        # === NEW: Replace research bibliography (page 15b) per-line ===
        if extra.get('biblio'):
            new_biblio = ''
            for title, body in extra['biblio']:
                new_biblio += f'''      <div class="callout" style="margin:0">
        <b class="title">{title}</b>
        <p style="font-size:11px">{body}</p>
      </div>
'''
            # Anchor by Research & Sources footer to disambiguate from use_cases section
            html = re.sub(
                r'(<div style="display:grid;grid-template-columns:repeat\(2,1fr\);gap:14px;margin-top:18px">\s*)(?:<div class="callout" style="margin:0">.*?</div>\s*){7,9}(\s*</div>\s*<div class="callout dark" style="margin-top:20px">\s*<b class="title">Cómo accedes al research dossier</b>)',
                lambda m: m.group(1) + new_biblio + m.group(2),
                html, count=1, flags=re.DOTALL
            )

    # Save HTML
    out_html = ROOT / f'PEPTIQ-{cat_key}-B2B-Clinicas-2026.html'
    out_html.write_text(html, encoding='utf-8')

    # Generate PDF
    out_pdf = ROOT / f'PEPTIQ-{cat_key}-B2B-Clinicas-2026.pdf'
...
for cat_key in CATEGORIES.keys():
    pdf = ROOT / f'PEPTIQ-{cat_key}-B2B-Clinicas-2026.pdf'
    if pdf.exists():
